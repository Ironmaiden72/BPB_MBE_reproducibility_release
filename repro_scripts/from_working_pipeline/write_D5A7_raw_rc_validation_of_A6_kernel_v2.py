...
def _extract_from_zip_if_needed() -> Optional[Path]:
    """Extract required raw SPARC inputs from a local results.zip, when present.

    This keeps A7 reproducible on a clean checkout where results/chains was not
    expanded yet.  It never downloads anything; it only uses zip files already
    present locally.
    """
    zip_candidates = [
        _env_path("BPB_RESULTS_ZIP"),
        ROOT / "results.zip",
        ROOT.parent / "results.zip",
        ROOT / "outputs" / "results.zip",
        Path("/mnt/data/results.zip"),  # harmless outside ChatGPT sandbox if absent
    ]
    members = [
        "results/chains/MassModels_Lelli2016c.mrt",
        "results/chains/sparc_reference_halo_table_nfw_lcdm_clean.csv",
        "results/chains/sparc_active_sample.csv",
    ]
    cache = OUT_DIR / "_raw_sparc_inputs"
...
def _find_file_exact(filename: str, roots: List[Path]) -> Optional[Path]:
    # 1) direct known directories
    for d in roots:
        if d is None:
            continue
        p = d / filename
        if p.exists():
            return p
    # 2) bounded recursive fallback under likely data/output roots
    recursive_roots = []
    for d in roots:
        if d is not None and d.exists() and d.is_dir() and d not in recursive_roots:
            recursive_roots.append(d)
    for root in recursive_roots:
        try:
            ...
            hits = []
        if hits:
            # Prefer results/chains when there are duplicates.
            hits = sorted(hits, key=lambda x: ("results/chains" not in str(x).replace("\\", "/"), len(str(x))))
            return hits[0]
    return None


def locate_raw_inputs() -> Dict[str, Path]:
    # Allow explicit override if the raw files live outside the repo.
    env_mrt = _env_path("SPARC_MRT")
    env_ref = _env_path("SPARC_REF")
    env_active = _env_path("SPARC_ACTIVE")
    env_dir = _env_path("SPARC_RAW_DIR")

    extracted_dir = _extract_from_zip_if_needed()
    ...
    mrt = env_mrt if env_mrt and env_mrt.exists() else _find_file_exact("MassModels_Lelli2016c.mrt", chains_candidates)
    ref = env_ref if env_ref and env_ref.exists() else _find_file_exact("sparc_reference_halo_table_nfw_lcdm_clean.csv", chains_candidates)
    active = env_active if env_active and env_active.exists() else _find_file_exact("sparc_active_sample.csv", chains_candidates)

    missing = []
    if mrt is None:
        missing.append("MassModels_Lelli2016c.mrt")
    if ref is None:
        missing.append("sparc_reference_halo_table_nfw_lcdm_clean.csv")
    if active is None:
        missing.append("sparc_active_sample.csv")
    if missing:
        tried = "\n".join(str(d) for d in chains_candidates if d is not None)
        raise FileNotFoundError(
            "Could not locate required raw SPARC A7 inputs: " + ", ".join(missing) +
            "\nTried known dirs and recursive fallbacks under:\n" + tried +
            "\n\nFix options:\n"
            "  1) unzip results.zip at repo root so results/chains/... exists; or\n"
            "  2) export SPARC_RAW_DIR=/path/to/dir containing the three files; or\n"
            "  3) export SPARC_MRT, SPARC_REF, and SPARC_ACTIVE explicitly."
        )
